...
fn start_dashing(
    ...
    direction: Vec3,
    dasher: &Dasher,
    original_collision_groups: &CollisionLayers,
    ...
    if direction != Vec3::ZERO {
        ...
        commands.entity(entity).insert((
            ...
            CollisionLayers::new(
                ...
                original_collision_groups.filters & !LayerMask::from(GameLayers::Bullet),
            ),
        ));
    }
}
...
fn handle_active_dash(
    mut query: Query<(Entity, &mut ActiveDash, &mut LinearVelocity)>,
    time: Res<Time>,
    mut commands: Commands,
) {
    for (entity, mut dash, mut velocity) in query.iter_mut() {
        // TODO: Extract movement to FixedUpdate (TODO: Is it still relevant in avian?)
        velocity.0 = dash.linear_velocity;
